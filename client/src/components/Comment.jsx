...
export default function Comment({comment, onLike, onEdit, onDelete}) {
  ...
  const [editedContent, setEditedContent] = useState(comment.content)
    useEffect(()=>{
        const getUser = async ()=>{
            const res = await fetch(`https://mern-blog-api-sepia.vercel.app/api/user/${comment.userId}`)
    ...
    const handleSave = async()=>{
      try{
        const res = await fetch(`https://mern-blog-api-sepia.vercel.app/api/comment/editcomment/${comment._id}`,{
          method:'PUT',
          headers: {
  ...
  return (
    ...
    <div className='flex-1'>
      ...
      <div className='flex items-cente pt-2 text-xs border-t dark:border-gray-700 max-w-fit gap-2 '>
